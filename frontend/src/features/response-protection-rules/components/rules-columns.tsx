...
function StatusSwitchCell({ row }: { row: Row<ResponseProtectionRule> }) {
...
export const createColumns = (t: ReturnType<typeof useTranslation>['t'], canWrite: boolean = true): ColumnDef<ResponseProtectionRule>[] => [
  ...(canWrite
    ? [
        {
          id: 'select',
          header: ({ table }: { table: Table<ResponseProtectionRule> }) => (
            <Checkbox
          ...
          cell: ({ row }: { row: Row<ResponseProtectionRule> }) => (
            <Checkbox
  ...
  {
    accessorKey: 'pattern',
    header: ({ column }) => <DataTableColumnHeader column={column} title={t('responseProtectionRules.columns.pattern')} />,
    cell: ({ row }) => (
      <Tooltip>
        <TooltipTrigger asChild>
  ...
  {
    id: 'action',
    header: ({ column }) => <DataTableColumnHeader column={column} title={t('responseProtectionRules.columns.action')} />,
    cell: ({ row }) => {
      const action = row.original.settings.action;
      const variant = action === 'reject' ? 'destructive' : action === 'failover' ? 'default' : 'secondary';

      return <Badge variant={variant}>{t(`responseProtectionRules.actions.${action}`)}</Badge>;
    },
    enableSorting: false,
  },
  {
    id: 'scopes',
    header: ({ column }) => <DataTableColumnHeader column={column} title={t('responseProtectionRules.columns.scopes')} />,
    cell: ({ row }) => {
