...
import { ResponseProtectionRule, ResponseProtectionRuleConnection } from '../data/schema';

interface RulesTableProps {
  columns: ColumnDef<ResponseProtectionRule>[];
  data: ResponseProtectionRule[];
  loading?: boolean;
  pageInfo?: ResponseProtectionRuleConnection['pageInfo'];
  pageSize: number;
  totalCount?: number;
  nameFilter: string;
...
export function RulesTable({
  ...
  const { setSelectedRules, setResetRowSelection, setOpen } = useResponseProtectionRules();
  ...
  useEffect(() => {
    setSelectedRules(filteredSelectedRows.map((row) => row.original as ResponseProtectionRule));
    ...
    <div className='flex flex-1 flex-col overflow-hidden'>
      <div className='mb-4 flex items-center justify-between'>
        <div className='flex flex-1 items-center space-x-2'>
          <Input
            placeholder={t('responseProtectionRules.filters.filterByName')}
