...
// CollectFields tells the query-builder to eagerly load connected nodes by resolver context.
func (_q *ResponseProtectionRuleQuery) CollectFields(ctx context.Context, satisfies ...string) (*ResponseProtectionRuleQuery, error) {
	fc := graphql.GetFieldContext(ctx)
	if fc == nil {
		return _q, nil
	}
...
func (_q *ResponseProtectionRuleQuery) collectField(ctx context.Context, oneNode bool, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	path = append([]string(nil), path...)
	var (
		unknownSeen    bool
		fieldSeen      = make(map[string]struct{}, len(responseprotectionrule.Columns))
		selectedFields = []string{responseprotectionrule.FieldID}
	)
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		switch field.Name {
		case "createdAt":
			if _, ok := fieldSeen[responseprotectionrule.FieldCreatedAt]; !ok {
				selectedFields = append(selectedFields, responseprotectionrule.FieldCreatedAt)
				fieldSeen[responseprotectionrule.FieldCreatedAt] = struct{}{}
			}
		case "updatedAt":
			if _, ok := fieldSeen[responseprotectionrule.FieldUpdatedAt]; !ok {
				selectedFields = append(selectedFields, responseprotectionrule.FieldUpdatedAt)
				fieldSeen[responseprotectionrule.FieldUpdatedAt] = struct{}{}
			}
		case "name":
			if _, ok := fieldSeen[responseprotectionrule.FieldName]; !ok {
				selectedFields = append(selectedFields, responseprotectionrule.FieldName)
				fieldSeen[responseprotectionrule.FieldName] = struct{}{}
			}
		case "description":
			if _, ok := fieldSeen[responseprotectionrule.FieldDescription]; !ok {
				selectedFields = append(selectedFields, responseprotectionrule.FieldDescription)
				fieldSeen[responseprotectionrule.FieldDescription] = struct{}{}
			}
		case "pattern":
			if _, ok := fieldSeen[responseprotectionrule.FieldPattern]; !ok {
				selectedFields = append(selectedFields, responseprotectionrule.FieldPattern)
				fieldSeen[responseprotectionrule.FieldPattern] = struct{}{}
			}
		case "status":
			if _, ok := fieldSeen[responseprotectionrule.FieldStatus]; !ok {
				selectedFields = append(selectedFields, responseprotectionrule.FieldStatus)
				fieldSeen[responseprotectionrule.FieldStatus] = struct{}{}
			}
		case "settings":
			if _, ok := fieldSeen[responseprotectionrule.FieldSettings]; !ok {
				selectedFields = append(selectedFields, responseprotectionrule.FieldSettings)
				fieldSeen[responseprotectionrule.FieldSettings] = struct{}{}
			}
		case "id":
		case "__typename":
		default:
...
type responseprotectionrulePaginateArgs struct {
	first, last   *int
	after, before *Cursor
	opts          []ResponseProtectionRulePaginateOption
}

func newResponseProtectionRulePaginateArgs(rv map[string]any) *responseprotectionrulePaginateArgs {
	args := &responseprotectionrulePaginateArgs{}
	if rv == nil {
		return args
	}
	if v := rv[firstField]; v != nil {
	...
	if v, ok := rv[orderByField]; ok {
		...
		case map[string]any:
			var (
				err1, err2 error
				order      = &ResponseProtectionRuleOrder{Field: &ResponseProtectionRuleOrderField{}, Direction: entgql.OrderDirectionAsc}
			)
			if d, ok := v[directionField]; ok {
				err1 = order.Direction.UnmarshalGQL(d)
			}
			...
			if err1 == nil && err2 == nil {
				args.opts = append(args.opts, WithResponseProtectionRuleOrder(order))
			}
		case *ResponseProtectionRuleOrder:
			if v != nil {
				args.opts = append(args.opts, WithResponseProtectionRuleOrder(v))
			}
		}
	}
	if v, ok := rv[whereField].(*ResponseProtectionRuleWhereInput); ok {
		args.opts = append(args.opts, WithResponseProtectionRuleFilter(v.Filter))
	}
	return args
}
