...
// ResponseProtectionRuleEdge is the edge representation of ResponseProtectionRule.
type ResponseProtectionRuleEdge struct {
	Node   *ResponseProtectionRule `json:"node"`
	Cursor Cursor                  `json:"cursor"`
}

// ResponseProtectionRuleConnection is the connection containing edges to ResponseProtectionRule.
type ResponseProtectionRuleConnection struct {
	Edges      []*ResponseProtectionRuleEdge `json:"edges"`
	PageInfo   PageInfo                      `json:"pageInfo"`
	TotalCount int                           `json:"totalCount"`
}

func (c *ResponseProtectionRuleConnection) build(nodes []*ResponseProtectionRule, pager *responseprotectionrulePager, after *Cursor, first *int, before *Cursor, last *int) {
	c.PageInfo.HasNextPage = before != nil
	c.PageInfo.HasPreviousPage = after != nil
	if first != nil && *first+1 == len(nodes) {
		c.PageInfo.HasNextPage = true
	...
	var nodeAt func(int) *ResponseProtectionRule
	if last != nil {
		n := len(nodes) - 1
		nodeAt = func(i int) *ResponseProtectionRule {
			return nodes[n-i]
		}
	} else {
		nodeAt = func(i int) *ResponseProtectionRule {
	...
	c.Edges = make([]*ResponseProtectionRuleEdge, len(nodes))
	for i := range nodes {
		node := nodeAt(i)
		c.Edges[i] = &ResponseProtectionRuleEdge{
			Node:   node,
			Cursor: pager.toCursor(node),
		}
	}
...
// ResponseProtectionRulePaginateOption enables pagination customization.
type ResponseProtectionRulePaginateOption func(*responseprotectionrulePager) error

// WithResponseProtectionRuleOrder configures pagination ordering.
func WithResponseProtectionRuleOrder(order *ResponseProtectionRuleOrder) ResponseProtectionRulePaginateOption {
	if order == nil {
		order = DefaultResponseProtectionRuleOrder
	}
	o := *order
	return func(pager *responseprotectionrulePager) error {
		if err := o.Direction.Validate(); err != nil {
			return err
		}
		if o.Field == nil {
			o.Field = DefaultResponseProtectionRuleOrder.Field
		}
		pager.order = &o
		return nil
	}
...
// WithResponseProtectionRuleFilter configures pagination filter.
func WithResponseProtectionRuleFilter(filter func(*ResponseProtectionRuleQuery) (*ResponseProtectionRuleQuery, error)) ResponseProtectionRulePaginateOption {
	return func(pager *responseprotectionrulePager) error {
		if filter == nil {
			return errors.New("ResponseProtectionRuleQuery filter cannot be nil")
		}
		pager.filter = filter
		return nil
	}
...
type responseprotectionrulePager struct {
	reverse bool
	order   *ResponseProtectionRuleOrder
	filter  func(*ResponseProtectionRuleQuery) (*ResponseProtectionRuleQuery, error)
}

func newResponseProtectionRulePager(opts []ResponseProtectionRulePaginateOption, reverse bool) (*responseprotectionrulePager, error) {
	pager := &responseprotectionrulePager{reverse: reverse}
	for _, opt := range opts {
		if err := opt(pager); err != nil {
			return nil, err
		}
	...
	if pager.order == nil {
		pager.order = DefaultResponseProtectionRuleOrder
	}
	return pager, nil
}

func (p *responseprotectionrulePager) applyFilter(query *ResponseProtectionRuleQuery) (*ResponseProtectionRuleQuery, error) {
	if p.filter != nil {
		return p.filter(query)
	}
	return query, nil
...
func (p *responseprotectionrulePager) toCursor(_m *ResponseProtectionRule) Cursor {
	return p.order.Field.toCursor(_m)
}

func (p *responseprotectionrulePager) applyCursors(query *ResponseProtectionRuleQuery, after, before *Cursor) (*ResponseProtectionRuleQuery, error) {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	for _, predicate := range entgql.CursorsPredicate(after, before, DefaultResponseProtectionRuleOrder.Field.column, p.order.Field.column, direction) {
		query = query.Where(predicate)
	}
	return query, nil
}

func (p *responseprotectionrulePager) applyOrder(query *ResponseProtectionRuleQuery) *ResponseProtectionRuleQuery {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	query = query.Order(p.order.Field.toTerm(direction.OrderTermOption()))
	if p.order.Field != DefaultResponseProtectionRuleOrder.Field {
		query = query.Order(DefaultResponseProtectionRuleOrder.Field.toTerm(direction.OrderTermOption()))
	}
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(p.order.Field.column)
	}
...
func (p *responseprotectionrulePager) orderExpr(query *ResponseProtectionRuleQuery) sql.Querier {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	...
	return sql.ExprFunc(func(b *sql.Builder) {
		b.Ident(p.order.Field.column).Pad().WriteString(string(direction))
		if p.order.Field != DefaultResponseProtectionRuleOrder.Field {
			b.Comma().Ident(DefaultResponseProtectionRuleOrder.Field.column).Pad().WriteString(string(direction))
		}
	})
}

// Paginate executes the query and returns a relay based cursor connection to ResponseProtectionRule.
func (_m *ResponseProtectionRuleQuery) Paginate(
	ctx context.Context, after *Cursor, first *int,
	before *Cursor, last *int, opts ...ResponseProtectionRulePaginateOption,
) (*ResponseProtectionRuleConnection, error) {
	if err := validateFirstLast(first, last); err != nil {
		return nil, err
	}
	pager, err := newResponseProtectionRulePager(opts, last != nil)
	if err != nil {
		return nil, err
	}
	if _m, err = pager.applyFilter(_m); err != nil {
	...
	conn := &ResponseProtectionRuleConnection{Edges: []*ResponseProtectionRuleEdge{}}
	ignoredEdges := !hasCollectedField(ctx, edgesField)
	if hasCollectedField(ctx, totalCountField) || hasCollectedField(ctx, pageInfoField) {
		hasPagination := after != nil || first != nil || before != nil || last != nil
		if hasPagination || ignoredEdges {
...
var (
	// ResponseProtectionRuleOrderFieldCreatedAt orders ResponseProtectionRule by created_at.
	ResponseProtectionRuleOrderFieldCreatedAt = &ResponseProtectionRuleOrderField{
		Value: func(_m *ResponseProtectionRule) (ent.Value, error) {
			return _m.CreatedAt, nil
		},
		column: responseprotectionrule.FieldCreatedAt,
		toTerm: responseprotectionrule.ByCreatedAt,
		toCursor: func(_m *ResponseProtectionRule) Cursor {
			return Cursor{
				ID:    _m.ID,
				Value: _m.CreatedAt,
			}
	...
	// ResponseProtectionRuleOrderFieldUpdatedAt orders ResponseProtectionRule by updated_at.
	ResponseProtectionRuleOrderFieldUpdatedAt = &ResponseProtectionRuleOrderField{
		Value: func(_m *ResponseProtectionRule) (ent.Value, error) {
			return _m.UpdatedAt, nil
		},
		column: responseprotectionrule.FieldUpdatedAt,
		toTerm: responseprotectionrule.ByUpdatedAt,
		toCursor: func(_m *ResponseProtectionRule) Cursor {
			return Cursor{
				ID:    _m.ID,
				Value: _m.UpdatedAt,
	...
	// ResponseProtectionRuleOrderFieldName orders ResponseProtectionRule by name.
	ResponseProtectionRuleOrderFieldName = &ResponseProtectionRuleOrderField{
		Value: func(_m *ResponseProtectionRule) (ent.Value, error) {
			return _m.Name, nil
		},
		column: responseprotectionrule.FieldName,
		toTerm: responseprotectionrule.ByName,
		toCursor: func(_m *ResponseProtectionRule) Cursor {
			return Cursor{
				ID:    _m.ID,
				Value: _m.Name,
...
// String implement fmt.Stringer interface.
func (f ResponseProtectionRuleOrderField) String() string {
	var str string
	switch f.column {
	case ResponseProtectionRuleOrderFieldCreatedAt.column:
		str = "CREATED_AT"
	case ResponseProtectionRuleOrderFieldUpdatedAt.column:
		str = "UPDATED_AT"
	case ResponseProtectionRuleOrderFieldName.column:
		str = "NAME"
	}
	return str
}
...
// MarshalGQL implements graphql.Marshaler interface.
func (f ResponseProtectionRuleOrderField) MarshalGQL(w io.Writer) {
	io.WriteString(w, strconv.Quote(f.String()))
}

// UnmarshalGQL implements graphql.Unmarshaler interface.
func (f *ResponseProtectionRuleOrderField) UnmarshalGQL(v interface{}) error {
	str, ok := v.(string)
	if !ok {
		return fmt.Errorf("ResponseProtectionRuleOrderField %T must be a string", v)
	}
	switch str {
	case "CREATED_AT":
		*f = *ResponseProtectionRuleOrderFieldCreatedAt
	case "UPDATED_AT":
		*f = *ResponseProtectionRuleOrderFieldUpdatedAt
	case "NAME":
		*f = *ResponseProtectionRuleOrderFieldName
	default:
		return fmt.Errorf("%s is not a valid ResponseProtectionRuleOrderField", str)
	}
	return nil
}

// ResponseProtectionRuleOrderField defines the ordering field of ResponseProtectionRule.
type ResponseProtectionRuleOrderField struct {
	// Value extracts the ordering value from the given ResponseProtectionRule.
	Value    func(*ResponseProtectionRule) (ent.Value, error)
	column   string // field or computed.
	toTerm   func(...sql.OrderTermOption) responseprotectionrule.OrderOption
	toCursor func(*ResponseProtectionRule) Cursor
}

// ResponseProtectionRuleOrder defines the ordering of ResponseProtectionRule.
type ResponseProtectionRuleOrder struct {
	Direction OrderDirection                    `json:"direction"`
	Field     *ResponseProtectionRuleOrderField `json:"field"`
}

// DefaultResponseProtectionRuleOrder is the default ordering of ResponseProtectionRule.
var DefaultResponseProtectionRuleOrder = &ResponseProtectionRuleOrder{
	Direction: entgql.OrderDirectionAsc,
	Field: &ResponseProtectionRuleOrderField{
		Value: func(_m *ResponseProtectionRule) (ent.Value, error) {
			return _m.ID, nil
		},
		column: responseprotectionrule.FieldID,
		toTerm: responseprotectionrule.ByID,
		toCursor: func(_m *ResponseProtectionRule) Cursor {
			return Cursor{ID: _m.ID}
		},
	},
...
// ToEdge converts ResponseProtectionRule into ResponseProtectionRuleEdge.
func (_m *ResponseProtectionRule) ToEdge(order *ResponseProtectionRuleOrder) *ResponseProtectionRuleEdge {
	if order == nil {
		order = DefaultResponseProtectionRuleOrder
	}
	return &ResponseProtectionRuleEdge{
		Node:   _m,
		Cursor: order.Field.toCursor(_m),
	}
}
