...
// The ResponseProtectionRuleFunc type is an adapter to allow the use of ordinary
// function as ResponseProtectionRule mutator.
type ResponseProtectionRuleFunc func(context.Context, *ent.ResponseProtectionRuleMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ResponseProtectionRuleFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ResponseProtectionRuleMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ResponseProtectionRuleMutation", m)
}
