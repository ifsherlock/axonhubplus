...
// The ResponseProtectionRuleFunc type is an adapter to allow the use of ordinary function as a Querier.
type ResponseProtectionRuleFunc func(context.Context, *ent.ResponseProtectionRuleQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f ResponseProtectionRuleFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.ResponseProtectionRuleQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.ResponseProtectionRuleQuery", q)
}

// The TraverseResponseProtectionRule type is an adapter to allow the use of ordinary function as Traverser.
type TraverseResponseProtectionRule func(context.Context, *ent.ResponseProtectionRuleQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseResponseProtectionRule) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseResponseProtectionRule) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.ResponseProtectionRuleQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.ResponseProtectionRuleQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.ResponseProtectionRuleQuery:
		return &query[*ent.ResponseProtectionRuleQuery, predicate.ResponseProtectionRule, responseprotectionrule.OrderOption]{typ: ent.TypeResponseProtectionRule, tq: q}, nil
	case *ent.RoleQuery:
