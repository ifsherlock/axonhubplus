...
var responseprotectionruleImplementors = []string{"ResponseProtectionRule", "Node"}

// IsNode implements the Node interface check for GQLGen.
func (*ResponseProtectionRule) IsNode() {}
...
func (c *Client) noder(ctx context.Context, table string, id int) (Noder, error) {
	...
	case requestexecution.Table:
		...
		return query.Only(ctx)
	case responseprotectionrule.Table:
		query := c.ResponseProtectionRule.Query().
			Where(responseprotectionrule.ID(id))
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, responseprotectionruleImplementors...); err != nil {
				return nil, err
			}
		}
		return query.Only(ctx)
...
func (c *Client) noders(ctx context.Context, table string, ids []int) ([]Noder, error) {
	...
	case responseprotectionrule.Table:
		...
		query, err := query.CollectFields(ctx, responseprotectionruleImplementors...)
		if err != nil {
			return nil, err
		}
		nodes, err := query.All(ctx)
