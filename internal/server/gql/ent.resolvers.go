...
// ResponseProtectionRules is the resolver for the responseProtectionRules field.
func (r *queryResolver) ResponseProtectionRules(ctx context.Context, after *entgql.Cursor[int], first *int, before *entgql.Cursor[int], last *int, orderBy *ent.ResponseProtectionRuleOrder, where *ent.ResponseProtectionRuleWhereInput) (*ent.ResponseProtectionRuleConnection, error) {
	if err := validatePaginationArgs(first, last); err != nil {
		return nil, err
	}

	if orderBy != nil && orderBy.Field.String() == "CREATED_AT" {
		orderBy.Field = ent.DefaultResponseProtectionRuleOrder.Field
	}

	return r.client.ResponseProtectionRule.Query().Paginate(ctx, after, first, before, last,
		ent.WithResponseProtectionRuleOrder(orderBy),
		ent.WithResponseProtectionRuleFilter(where.Filter),
	)
}
...
// ResponseProtectionRule returns ResponseProtectionRuleResolver implementation.
func (r *Resolver) ResponseProtectionRule() ResponseProtectionRuleResolver {
	return &responseProtectionRuleResolver{r}
}
...
type responseProtectionRuleResolver struct{ *Resolver }
